...
| **3** | Playable with chapter handoff, mini-game, pathfinding, texture, and runtime fixes. Further room-by-room optimization continues. |
| **4** | Playable with runtime and interface compatibility fixes. Additional full-run testing remains planned. |
...
The patchers require an official, unmodified DELTARUNE installation. v0.69 supports Steam **v0.0.253**.

> [!IMPORTANT]
> v0.69 requires freshly generated data. Do not reuse a data folder generated by an older patcher.
...
<div align="center">
  ...
  ### DELTARUNE for PS Vita (v0.69 Showcase)

  <a href="https://youtu.be/41NiqU7HHVg">
    <img src="https://img.youtube.com/vi/41NiqU7HHVg/maxresdefault.jpg" width="85%" style="border-radius: 10px; box-shadow: 0 8px 24px rgba(0,0,0,0.5);" alt="DELTARUNE Vita v0.69 Showcase">
  </a>
  <a href="https://youtu.be/41NiqU7HHVg">
...
Prepared atlases are stored outside the VPK under `ux0:data/deltarune/deltarunevita/`. Language packs keep their own texture set whenever their `data.win` changes atlas contents or indices; English pages are not reused blindly for PT-BR or Spanish. Fonts and other precision-sensitive pages may still use the safer prepared cache when required.
...
Chapter 3 and Chapter 5 videos are decoded through the PS Vita `SceAvPlayer`. Original and translated `data.win` files resolve MP4 names through the same shared directory:
...
- Configurable prepared-texture profiles, including the validated BC3/DXT5 path.
...
Optional language packages currently used by the patchers:

- [Portuguese-BR translation package](https://drive.google.com/file/d/1WB1NLqAOE2xByes1ZQSeUo-i2HuPrwSm/view?usp=sharing)
- [Spanish translation package](https://drive.google.com/file/d/1u7Ikk1B_44J06Jo6lM6Rdj4uQonsbbOa/view?usp=sharing), based on deltaesp and supplied by [JBLeopard](https://github.com/JBLeopard)

After obtaining a translation from its original project, place it inside `mods/Lang/<Language>` (for example, `mods/Lang/Portuguese-BR`) and run:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\prepare-vita-mods.ps1
```
> [!NOTE]
> v0.69 includes initial Spanish support using the deltaesp translation supplied by [JBLeopard](https://github.com/JBLeopard). Language packs keep their own data and texture pages to prevent atlas-index mismatches.
...
- The interface reuses the game's dialogue border and text style for visual consistency.
