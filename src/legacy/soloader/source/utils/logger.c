...
static void log_write_file(const char *message) {
    SceUID fd = sceIoOpen(DATA_PATH "port.log",
                         SCE_O_WRONLY | SCE_O_CREAT | SCE_O_APPEND, 0666);
    if (fd >= 0) {
        sceIoWrite(fd, message, sceClibStrnlen(message, 2048));
        sceIoClose(fd);
    }
}
...
void log_marker(const char *message) {
    char line[512];
