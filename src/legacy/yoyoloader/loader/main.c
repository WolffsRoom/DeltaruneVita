...
#if 1
static char debug_log_path[512] = "ux0:data/gms/shared/yyl.log";

static void init_port_log(const char *game) {
#ifdef STANDALONE_MODE
	sprintf(debug_log_path, "ux0:data/gms/port.log");
#else
	sprintf(debug_log_path, "%s/%s/port.log", DATA_PATH, game);
#endif
...
int debugPrintf(char *text, ...) {
	...
	SceUID fd = sceIoOpen(debug_log_path, SCE_O_WRONLY | SCE_O_CREAT | SCE_O_APPEND, 0777);
	if (fd >= 0) {
	...
	case CALL_EXTENSION_FUNCTION:
		{
			...
			debugPrintf("Extension call module=%s method=%s argc=%d\n",
				f->module_name ? f->module_name : "(null)",
				f->method_name ? f->method_name : "(null)", f->argc);
...
void *pthread_main(void *arg) {
	...
	init_port_log(game_name);
	debugPrintf("BUILD_ID=%s\n", DELTARUNE_PORT_BUILD_ID);
	debugPrintf("TARGET_APK_SHA256=%s\n", DELTARUNE_APK_SHA256);
	debugPrintf("TARGET_APK_SIZE=%llu\n", DELTARUNE_APK_SIZE);
	debugPrintf("GAME_NAME=%s\n", game_name);
	debugPrintf("APK_PATH=%s\n", apk_path);
	debugPrintf("+--------------------------------------------+\n");
